#!/usr/bin/env bash
#
# installers/installer_master.sh — Master installer / deployer / verifier for AI Agent
...
# - Max-logic: Parse → Cross-reference → Infer → Test → Reconstruct → Validate → Output
#
# Usage examples:
#   sudo ./installers/installer_master.sh --non-interactive --auto-repair
#   sudo ./installers/installer_master.sh --kube-deploy --kubeconfig ~/.kube/config --namespace aiagent --image-pull-secret my-registry-secret --production-verify "kubectl -n aiagent get all"
#
set -o errexit
set -o nounset
set -o pipefail
...
# ---------------------------
# Paths & runtime env (dynamic)
# ---------------------------
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd -P)"
LOGFILE="/var/log/ai-agent-installer-master.log"
mkdir -p "$(dirname "$LOGFILE")"
touch "$LOGFILE"
exec 3>>"$LOGFILE"  # fd3 appended logs
...
HEALTHCHECK_MODULE="$REPO_ROOT/devops/tools/healthcheck.sh"
UI_MODULE="$REPO_ROOT/ai-agent/modules/ui.sh"
INSTALLER_LOCAL="$REPO_ROOT/installers/installer_local.sh"   # local installer
INSTALLER_KUBE="$REPO_ROOT/installers/installer_kube.sh"     # kube installer
AUTO_BOOTSTRAP="$REPO_ROOT/auto-bootstrap-aiagent.sh"
...
save_snapshot(){
  ...
  command -v jq >/dev/null 2>&1 || ( echo '{"error":"jq-missing"}' > "$out" && return 0 )
  jq -n \
...
usage(){
  cat <<EOF
installers/installer_master.sh - master installer & deployer

Usage:
  sudo ./installers/installer_master.sh [options]
...
if [ "$ULTRA_MODE" = "yes" ]; then
  RETRY_BASE=6; RETRY_MAX=600; APT_TIMEOUT=180
...
# Try to source UI and healthcheck modules for nicer output, if available
if [ -f "$UI_MODULE" ]; then
  # shellcheck disable=SC1091
  source "$UI_MODULE" || true
fi
if [ -f "$HEALTHCHECK_MODULE" ]; then
  # shellcheck disable=SC1091
  source "$HEALTHCHECK_MODULE" || true
fi
...
install_base_packages(){
  ...
  set -e
  if [ $rc -ne 0 ]; then
    warn "apt-get update failed (rc=$rc) — trying dpkg repair helper if available"
  ...
  warn "Attempting emergency tool fallback (wget/deb) if present"
  if [ -x "$EMERGENCY_TOOL" ]; then
    bash "$EMERGENCY_TOOL" --full-auto --wget-fallback --non-interactive >> "$LOGFILE" 2>&1 || true
  else
    warn "Emergency tool missing"
  fi

  if retry_limited 2 "DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends ${ALL_APT_PACKAGES[*]} >> \"$LOGFILE\" 2>&1"; then
    ok "Base packages installed on final attempt"
    return 0
  fi
...
# -----------------------
# Run local generators & builders
# -----------------------
run_local_helpers(){
  info "Running repo helper scripts"

  if [ -f "$GEN_DEVOPS" ] && [ -x "$GEN_DEVOPS" ]; then
    run_cmd bash "$GEN_DEVOPS" || warn "generate_devops produced warnings"
  else
    warn "generate_devops script missing or not executable: $GEN_DEVOPS"
  fi

  if [ -f "$BUILD_BUNDLE" ] && [ -x "$BUILD_BUNDLE" ]; then
    run_cmd bash "$BUILD_BUNDLE" || warn "build_and_bundle produced warnings"
  else
    warn "build_and_bundle script missing or not executable: $BUILD_BUNDLE"
  fi

  # optionally run auto-bootstrap
  if [ -f "$AUTO_BOOTSTRAP" ] && [ -x "$AUTO_BOOTSTRAP" ]; then
    run_cmd bash "$AUTO_BOOTSTRAP" || warn "auto-bootstrap produced warnings"
  fi
}

# -----------------------
# Helm & kubectl install (if needed)
# -----------------------
install_helm_and_prereqs(){
  info "Ensuring Helm and kubectl are present"
  if ! command -v helm >/dev/null 2>&1; then
    info "Installing Helm (script)"
  ...
  if ! command -v kubectl >/dev/null 2>&1; then
    ...
    local krel
    krel=$(curl -L -s https://dl.k8s.io/release/stable.txt || echo "")
    if [ -n "$krel" ]; then
      retry_limited 3 "curl -LO https://dl.k8s.io/release/${krel}/bin/linux/amd64/kubectl >> \"$LOGFILE\" 2>&1 || true"
      retry_limited 3 "install -o root -g root -m 0755 kubectl /usr/local/bin/kubectl >> \"$LOGFILE\" 2>&1 || true"
    else
      warn "Could not discover stable kubectl release; please install manually."
    fi
  fi
...
helm_deploy_chart(){
  ...
  # Ensure cert-manager and ingress-nginx are installed? (best-effort)
  info "Ensuring cert-manager and ingress-nginx (best-effort — prefer cluster admin)"
  # Do not auto-install cluster-level components without explicit consent in production
  # (the oneclick script handles bootstrapping if desired)

  # imagePullSecret handling (assumes secret already created)
  if [ -n "$IMAGE_PULL_SECRET" ]; then
    info "Linking imagePullSecret: $IMAGE_PULL_SECRET"
...
wait_for_k8s_resources(){
  ...
  while true; do
    ...
    if [ -z "$notready" ]; then
      ...
      warn "Pods not ready yet (showing up to 6 lines):"
      echo "$notready" | sed -n '1,6p'
    fi
    now=$(date +%s)
...
run_production_verify(){
  if [ -z "$PRODUCTION_VERIFY_CMD" ]; then
    warn "No production verify command supplied; skipping"
    return 0
  fi
...
run_post_deploy_smoke(){
  if [ -x "$POST_DEPLOY_SMOKE" ]; then
    ...
    # fallback: basic curl check (local)
    if command -v curl >/dev/null 2>&1 && [ "$KUBEDEPLOY" != "yes" ]; then
      if curl -fsS --max-time 5 http://127.0.0.1:8000/healthz >/dev/null 2>&1; then
        ok "Local health endpoint OK"
      else
        warn "Local health endpoint failed"
      fi
    fi
  fi
}
...
# -----------------------
local_install_flow(){
  info "Non-kube install: running local installer"

  # run local installer if present
  if [ -f "$INSTALLER_LOCAL" ] && [ -x "$INSTALLER_LOCAL" ]; then
    auto_repair_and_retry "run_local_installer" "bash \"$INSTALLER_LOCAL\" --non-interactive"
  else
    warn "Local installer missing: $INSTALLER_LOCAL; attempting fallback"
    if [ -f "$REPO_ROOT/installer.sh" ] && [ -x "$REPO_ROOT/installer.sh" ]; then
      auto_repair_and_retry "run_fallback_installer" "bash \"$REPO_ROOT/installer.sh\" --non-interactive"
    else
      warn "No local installer found to run"
    fi
  fi
...
main(){
  ...
  if [ "$KUBEDEPLOY" = "yes" ]; then
    # 3) If kube deploy requested: use installer_kube.sh
    if [ -f "$INSTALLER_KUBE" ] && [ -x "$INSTALLER_KUBE" ]; then
      local kube_args="--kubeconfig \"$KUBECONFIG\" --namespace \"$KUBE_NAMESPACE\""
      [ -n "$IMAGE_PULL_SECRET" ] && kube_args="$kube_args --image-pull-secret \"$IMAGE_PULL_SECRET\""
      [ -n "$HELM_VALUES_FILE" ] && kube_args="$kube_args --helm-values \"$HELM_VALUES_FILE\""
      auto_repair_and_retry "run_kube_installer" "bash \"$INSTALLER_KUBE\" $kube_args"
    else
      warn "Kube installer missing: $INSTALLER_KUBE; falling back to inline deployment"
      install_helm_and_prereqs
    ...
    # Local/systemd path: use installer_local.sh
    local_install_flow
  fi

  ok "Main install flow complete; running final verification"
...
# Run main and catch errors to attempt emergency repair
set +e
main
rc=$?
set -e
